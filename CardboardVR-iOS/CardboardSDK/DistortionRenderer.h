...
public:
    ...
    void onProjectionChanged(HeadMountedDisplay *hmd, EyeParams *leftEye, EyeParams *rightEye, float zNear, float zFar);
private:
...
private:
    ...
    DistortionMesh* createDistortionMesh(EyeParams *eye, EyeViewport eyeViewport, float textureWidthM, float textureHeightM, float xEyeOffsetMScreen, float yEyeOffsetMScreen);
    void renderDistortionMesh(DistortionMesh *mesh);
    float computeDistortionScale(Distortion *distortion, float screenWidthM, float interpupillaryDistanceM);
    int createTexture(int width, int height);
    int setupRenderTextureAndRenderbuffer(int width, int height);
    int loadShader(GLenum shaderType, const GLchar *source);
    int createProgram(const GLchar *vertexSource, const GLchar *fragmentSource);
